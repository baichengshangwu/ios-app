export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (authEmail && DB) {
      try {
        user = await DB.prepare('SELECT email, name, ac_balance, cny_balance, usd_balance FROM users WHERE email = ? AND pass = ?')
    ...
    // --- Fiat-only balance deduction: CNY first, then USD ---
    const CNY_RATE = 7.25;
    const cnyToDeduct = costUSD > 0 ? Math.round(costUSD * CNY_RATE * 100) / 100 : 0;
    const usdToDeduct = costUSD > 0 ? Math.round(costUSD * 100) / 100 : 0;

    let cnyAfter = null, usdAfter = null;
    let deducted = false;
    let deductType = 'none'; // 'cny' | 'usd'
    let deductError = null;

    if (user && DB && costUSD > 0) {
      if ((user.cny_balance || 0) >= cnyToDeduct) {
        try {
          cnyAfter = Math.round(((user.cny_balance || 0) - cnyToDeduct) * 100) / 100;
          await DB.prepare('UPDATE users SET cny_balance = ? WHERE email = ?')
            .bind(cnyAfter, authEmail).run();
          deducted = true;
          deductType = 'cny';
        } catch(e) { /* DB update failed */ }
      } else if ((user.usd_balance || 0) >= usdToDeduct) {
        try {
          usdAfter = Math.round(((user.usd_balance || 0) - usdToDeduct) * 100) / 100;
          await DB.prepare('UPDATE users SET usd_balance = ? WHERE email = ?')
            .bind(usdAfter, authEmail).run();
          deducted = true;
          deductType = 'usd';
        } catch(e) { /* DB update failed */ }
      } else {
        deductError = 'Insufficient fiat balance. Need ¥' + cnyToDeduct.toFixed(2) + ' CNY or $' + usdToDeduct.toFixed(2) + ' USD. Please recharge.';
      }
    }
    if (!deducted && user) {
      cnyAfter = user.cny_balance;
      usdAfter = user.usd_balance;
    }

    if (deductError) {
      return err(deductError, 402);
    }
      ...
      try {
        ...
        await DB.prepare(`
          INSERT INTO model_usage (id, user_email, model_id, model_name, vendor,
            ...
            ac_balance_after, cny_deducted, cny_balance_after, usd_deducted, usd_balance_after,
            exchange_rate, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
        `).bind(logId, authEmail, modelId, model, group,
          promptTokens, completionTokens, totalTokens, costUSD,
          0, null,
          deductType === 'cny' ? cnyToDeduct : 0, cnyAfter,
          deductType === 'usd' ? usdToDeduct : 0, usdAfter,
          null).run();
    ...
    return new Response(JSON.stringify({
      ...
      _ztw: {
        ...
        cny_deducted: deductType === 'cny' ? cnyToDeduct : 0,
        cny_balance: cnyAfter,
        usd_deducted: deductType === 'usd' ? usdToDeduct : 0,
        usd_balance: usdAfter,
